...
export default function Header({ variant = "home", title }: HeaderProps) {
  ...
  // 콘솔은 클라이언트에서만 — 이 컴포넌트 자체가 client이므로 OK
  if (typeof window !== "undefined") {
    console.log("현재 저장된 authToken:", localStorage.getItem("authToken"));
  }

  // ✅ home일 때만 /api/users/me 호출
  const { data: myName, isLoading } = useMyName(isHome);

  return (
    <header className="sticky top-0 w-full h-[60px] bg-white flex items-end px-5 pb-3 shadow-sm z-50">
      <div className="flex items-center justify-between w-full max-w-[480px] mx-auto">
        {isHome ? (
          ...
          <div className="flex items-center space-x-2">
            <button onClick={() => router.back()} className="w-7 h-7 flex items-center justify-center">
